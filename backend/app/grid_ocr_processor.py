...
class GridLineDetector:
    ...
    def detect_grid(
        ...
        kernel_ratios = [0.08, 0.12, 0.18, 0.25]
...
class GridOCRTableExtractor:
    ...
    def extract_data_only(
        ...
        # Validate: if col count is wildly different from expected, reject
        if self._expected_cols is not None:
            if grid.num_cols < self._expected_cols - 2 or grid.num_cols > self._expected_cols + 2:
                logger.warning(
                    f"Page {page_number}: Grid col count {grid.num_cols} doesn't match "
                    f"expected {self._expected_cols}, rejecting grid result"
                )
                return None
