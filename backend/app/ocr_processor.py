...
class OCRConfig:
    ...
    # Complex OCR / election mode: higher DPI and stricter structure preservation
    complex_ocr_mode: bool = False  # When True, use higher DPI (caller should set dpi=350)
...
class OCRProcessor:
    ...
    def extract_tables(
        ...
        first_page_column_boundaries = None  # (x_min, x_max) per column from page 1 bbox for stable alignment
        ...
        for page_idx, image in enumerate(images):
            ...
            if page_num == 1:
                ...
                if table_data and not table_data.is_empty:
                    ...
                    first_page_headers = table_data.headers
                    # Compute column boundaries from page 1 bbox for stable alignment on later pages
                    from .table_parser import OCRTableParser
                    _parser = OCRTableParser()
                    first_page_column_boundaries = _parser.compute_column_boundaries(
                        ocr_result.bounding_boxes, y_tolerance=15
                    )
                    if first_page_column_boundaries:
                        logger.debug(
                            "Page 1: computed %s column boundaries for subsequent pages",
                            len(first_page_column_boundaries),
                        )
                    # Optional AI structure hint when bbox fallback produced few columns (low confidence)
                    if len(first_page_headers) < 6:
                        try:
                            from .ocr_structure_ai import infer_table_structure
                            suggestion = infer_table_structure(
                                ocr_result.raw_text,
                                first_page_headers,
                                table_data.rows[:5] if table_data.rows else [],
                                page_number=1,
                            )
                            if suggestion and suggestion.get("num_columns", 0) > len(first_page_headers):
                                logger.warning(
                                    "OCR structure AI suggested %s columns (current: %s). "
                                    "Consider reviewing extraction.",
                                    suggestion["num_columns"], len(first_page_headers),
                                )
                        except Exception as e:
                            logger.debug("OCR structure AI skip: %s", e)
                    logger.info(
                ...
                # Parse OCR result but extract only data rows (use page 1 column boundaries if available)
                table_data = self._parse_ocr_data_only(
                    ocr_result, first_page_headers,
                    column_boundaries=first_page_column_boundaries,
                )
    ...
    def _parse_ocr_data_only(
        ...
        that appear on each page.         Optionally uses column_boundaries from
        page 1 for stable column alignment.

        Args:
            ocr_result: OCR result from a single page
            expected_headers: Headers from first page
            column_boundaries: Optional (x_min, x_max) per column from page 1
            y_tolerance: Y-coordinate tolerance for grouping into rows
        ...
        # Try election Form 20 format first, with optional column boundaries from page 1
        ...
        for row in full_table.rows:
            ...
            if len(normalized_row) < len(expected_headers):
                ...
                logger.debug(
                    "Page %s: trimming row from %s to %s columns (possible missing data)",
                    ocr_result.page_number, len(normalized_row), len(expected_headers),
                )
    ...
    def _merge_tables(self, tables: List[TableData]) -> Optional[TableData]:
        ...
        rows_trimmed = 0  # Rows that had more columns than headers (possible missing data)
        rows_padded = 0
        ...
        logger.info(
            f"Merged {len(tables)} tables: {len(all_rows)} total rows "
            f"(column-count: {len(headers)} cols)"
        )
        if rows_trimmed > 0:
            logger.warning(
                "OCR merge: %s row(s) had more columns than headers and were trimmed (possible missing data)",
                rows_trimmed,
            )
        if rows_padded > 0 and rows_padded != len(all_rows):
            logger.debug("OCR merge: %s row(s) padded to match header column count", rows_padded)
