...
import { uploadBoothPdf, subscribeToProgress, getStatus } from "@/lib/api";
...
export function useBoothUpload() {
  ...
  const uploadFile = useCallback(async (file: File) => {
    ...
    try {
      // Upload the file to booth endpoint
      const { task_id, filename } = await uploadBoothPdf(file);
